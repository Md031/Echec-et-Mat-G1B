...
if torch.cuda.is_available():
    ...
    device = "cuda"
    model.load_state_dict(torch.load('NeuralNetwork/ChessModel.pt'))
else: # for CPU-only machines
    print("Cuda is unavailable")
    device = "cpu"
    model.load_state_dict(torch.load('NeuralNetwork/ChessModel.pt', map_location=torch.device('cpu')))
...
for epoch_count, (X_input, y) in enumerate(data_train_loader): # X_input = 32 dispositions de board (batch)
    ...
    if epoch_count % 10 == 0: # Saves the model each 10 epoch
        torch.save(model.state_dict(), 'NeuralNetwork/ChessModel.pt', _use_new_zipfile_serialization=False) # Saves the model at each epoch